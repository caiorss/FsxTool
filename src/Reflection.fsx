...
module AsmAttr =
    ...
    /// Try find first type matching a name     
    let findType typeName (asm: Assembly) =
        let re = new Regex(typeName + "`")
        asm |> getTypes
            |> Seq.tryFind (fun t -> re.Match(typeName).Success)
...
module AsmDisplay =
    ...
    /// Try find type in GAC - mscorlib.dll
    ///
    let showTypeInfo tname =
        let errHandler () = printfn "Error: I can't find the type: %s" tname
        let asm = AsmAttr.load("mscorlib")
        AsmAttr.findType tname asm
